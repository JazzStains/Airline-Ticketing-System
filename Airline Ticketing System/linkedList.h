...
/************ADDED*****************/
    virtual void Swap_Nodes(Type& item1, Type& item2) = 0;
...
template <class Type>
const bool linkedListType<Type>::operator==
(const linkedListType<Type>& otherList)
{
    if (this != &otherList) //avoid self-copy
    {
...
template <class Type>
Type linkedListType<Type>::getKThElement(int k)
{
    assert(k <= count);
...
template <class Type>
void linkedListType<Type>::deteteKthElement(int k)
{
    assert(k <= count);
