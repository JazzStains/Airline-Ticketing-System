...
public:
  ...
  //*******ADDED********************************************//
    void Swap_Nodes(Type& item1, Type& item2);
...
bool unorderedLinkedList<Type>::search(const Type& searchItem) const
{
    ...
    while (current != NULL && !found)    //search the list
        if (current->info.id == searchItem.id) //searchItem is found
            found = true;
        else
...
void unorderedLinkedList<Type>::insertFirst(const Type& newItem)
{
    ...
    newNode->info.id = newItem.id;    //store the new item in the node
...
void unorderedLinkedList<Type>::deleteNode(const Type& deleteItem)
{
    ...
    else
    {
        ...
        else //search the list for the node with the given info
        {
            ...
            {
                if (current->info.id != deleteItem.id)
                {
...
//******************ADDEDD*****************
template <class Type>
void unorderedLinkedList<Type>::Swap_Nodes(Type& item1, Type& item2)
{
    nodeType<Type>* swap1, * swap2, * og;

    swap1 = this->first;
    swap2 = this->first;
    // node holders

    while ((swap1 != NULL))    //search the list
        if (swap1->info == item1 ) // check errors later
            break;
        else
            swap1 = swap1->link;
    ...
    while (swap2 != NULL)
        if (swap2->info == item2)
            ...
            swap2 = swap2->link;
    // all while loops locates the nodes where info == item1 and item2
    swap1->info = item2;
    swap2->info = item1;
